/*!
 * TradingVue.JS - v1.0.2 - Thu Nov 27 2025
...
var Grid = /*#__PURE__*/function () {
  ...
  createClass_createClass(Grid, [{
    ...
    value: function listeners() {
      ...
      mc.on('panstart', function (event) {
        ...
        if (_this.cursor.mode === 'aim') {
          ...
          if (_this.cursor.x != null && !isNaN(_this.cursor.x) && _this.cursor.y != null && !isNaN(_this.cursor.y)) {
            _this.aim_drag = {
              start_touch_x: event.center.x + _this.offset_x,
              start_touch_y: event.center.y + _this.offset_y,
              start_cross_x: _this.cursor.x,
              start_cross_y: _this.cursor.y
            };
          }
      ...
      mc.on('panmove', function (event) {
        ...
        if (_this.drug) {
        ...
        } else if (_this.cursor.mode === 'aim' && _this.aim_drag) {
          // Relative dragging: move crosshair by touch delta
          var current_touch_x = event.center.x + _this.offset_x;
          var current_touch_y = event.center.y + _this.offset_y;
          var delta_x = current_touch_x - _this.aim_drag.start_touch_x;
          var delta_y = current_touch_y - _this.aim_drag.start_touch_y;
          var new_cross_x = _this.aim_drag.start_cross_x + delta_x;
          var new_cross_y = _this.aim_drag.start_cross_y + delta_y; // Validate - don't emit if positions are NaN
          ...
          var clamped_x = Math.max(0, Math.min(_this.layout.width, new_cross_x));
          var clamped_y = Math.max(0, Math.min(_this.layout.height + _this.layout.offset, new_cross_y)); // Build event object with cursor position

          var cursorEvent = {
            mode: 'aim',
            // Explicitly maintain aim mode
            grid_id: _this.id,
            x: clamped_x,
            y: clamped_y,
          ...
          if (_this.cursor.measuring) {
            var layout = _this.$p.layout.grids[_this.id];
            var t = layout.screen2t(new_cross_x);
            var y$ = layout.screen2$(new_cross_y - _this.layout.offset);
          ...
          _this.comp.$emit('cursor-changed', cursorEvent);
        }
      });
      mc.on('panend', function (event) {
        ...
        if (_this.cursor.mode === 'aim' && _this.cursor.measuring && _this.aim_drag) {
          _this.measurement_panend_timestamp = utils.now();
        }
      ...
      mc.on('tap', function (event) {
        if (!utils.is_mobile) return; // Refresh cursor reference to ensure we have latest value

        _this.cursor = _this.comp.$props.cursor; // If in aim mode, handle measurement

        if (_this.cursor && _this.cursor.mode === 'aim') {
          // Ignore tap immediately after press when not measuring (prevent exit on finger lift)
          if (!_this.cursor.measuring && _this.press_timestamp && utils.now() - _this.press_timestamp < 500) {
            return;
          } // Ignore tap immediately after panend when measuring (finger lift after drag)


          if (_this.cursor.measuring && _this.measurement_panend_timestamp && utils.now() - _this.measurement_panend_timestamp < 150) {
            return;
          }
          ...
          if (!_this.cursor.measuring) {
            ...
            if (_this.cursor.m_p1) {
              _this.comp.$emit('cursor-changed', {
                mode: 'aim',
                x: _this.cursor.x,
                y: _this.cursor.y,
                handle_x: _this.cursor.handle_x,
                handle_y: _this.cursor.handle_y,
                measuring: false,
                m_p1: null,
                m_p2: null
              });
              ...
              _this.measurement_panend_timestamp = null;
              var t = layout.screen2t(_this.cursor.x);
              var y$ = layout.screen2$(_this.cursor.y - _this.layout.offset);
            ...
            var _t = layout.screen2t(_this.cursor.x);

            var _y$ = layout.screen2$(_this.cursor.y - _this.layout.offset);
            ...
            var finalEvent = {
              ...
              handle_y: _this.cursor.handle_y,
              measuring: false,
              // Lock measurement - can't drag anymore
              m_p1: _this.cursor.m_p1,
              // Keep m_p1 (first point)
              m_p2: [_t, _y$] // Update m_p2 (second point)

            };

            _this.comp.$emit('cursor-changed', finalEvent); // Measurement is now locked and displayed
            // User must long-press to exit aim mode and clear

          }

          _this.update(); // Prevent event from propagating to other handlers
      ...
      mc.on('press', function (event) {
        ...
        _this.calc_offset(); // Track press timestamp to prevent immediate tap trigger


        _this.press_timestamp = utils.now(); // Refresh cursor reference to ensure we have latest value

        _this.cursor = _this.comp.$props.cursor; // If already in aim mode, exit aim mode (whether actively measuring or has finalized measurement)
        ...
        if (_this.cursor.mode === 'aim') {
          ...
          if (_this.cursor.m_p1) exitEvent.m_p1 = _this.cursor.m_p1;
          if (_this.cursor.m_p2) exitEvent.m_p2 = _this.cursor.m_p2;

          _this.comp.$emit('cursor-changed', exitEvent);
        ...
        var touch_x = event.center.x + _this.offset_x;
        var touch_y = event.center.y + _this.offset_y + _this.layout.offset; // Build enter event - preserve existing measurement if any
        ...
        if (_this.cursor.m_p1) enterEvent.m_p1 = _this.cursor.m_p1;
        if (_this.cursor.m_p2) enterEvent.m_p2 = _this.cursor.m_p2;

        _this.comp.$emit('cursor-changed', enterEvent);
    ...
    key: "emit_cursor_coord_relative",
    value: function emit_cursor_coord_relative(handle_x, handle_y, cross_x, cross_y) {
      ...
      var clamped_x = Math.max(0, Math.min(this.layout.width, cross_x));
      var clamped_y = Math.max(0, Math.min(this.layout.height + this.layout.offset, cross_y));
      this.comp.$emit('cursor-changed', {
        grid_id: this.id,
        x: clamped_x,
        y: clamped_y,
...
var Crosshair = /*#__PURE__*/function () {
  ...
  createClass_createClass(Crosshair, [{
    ...
    value: function draw(ctx) {
      // Update reference to the grid
      this.layout = this.$p.layout;
      var cursor = this.comp.$props.cursor; // Always draw measurement box if it exists, even in explore mode

      this.draw_measurement(ctx); // Don't draw crosshair lines in explore mode if not visible
    ...
    key: "draw_measurement",
    value: function draw_measurement(ctx) {
      var cursor = this.$p.cursor; // Draw measurement box when we have both points (whether actively measuring or finalized)

      if (!cursor.m_p1 || !cursor.m_p2) return;
      var layout = this.$p.layout;
      var colors = this.$p.colors; // Convert points to screen coordinates

      var x1 = layout.t2screen(cursor.m_p1[0]);
      var y1 = layout.$2screen(cursor.m_p1[1]);
      var x2 = layout.t2screen(cursor.m_p2[0]);
      var y2 = layout.$2screen(cursor.m_p2[1]);
      ctx.save(); // Draw measurement box background

      ctx.fillStyle = colors.cross + '16'; // Semi-transparent

      ctx.fillRect(x1, y1, x2 - x1, y2 - y1); // Draw measurement box border (dashed)

      ctx.strokeStyle = colors.cross;
      ctx.lineWidth = 1;
      ctx.setLineDash([5, 5]);
      ctx.strokeRect(x1, y1, x2 - x1, y2 - y1); // Calculate center for dashed lines

      var xm = (x1 + x2) / 2;
      var ym = (y1 + y2) / 2; // Draw vertical dashed line
      // ctx.beginPath()
      // ctx.setLineDash([5, 5])
      // ctx.moveTo(xm, y1)
      // ctx.lineTo(xm, y2)
      ...
      ctx.moveTo(x1, ym);
      ctx.lineTo(x2, ym);
      ctx.stroke(); // Draw measurement values

      this.draw_measurement_values(ctx, cursor, layout, colors, x1, y1, x2, y2, xm);
      ctx.restore();
    }
  }, {
    key: "draw_measurement_values",
    value: function draw_measurement_values(ctx, cursor, layout, colors, x1, y1, x2, y2, xm) {
      // Calculate deltas
      var d$ = cursor.m_p2[1] - cursor.m_p1[1];
      var p = (100 * (cursor.m_p2[1] / cursor.m_p1[1] - 1)).toFixed(2); // Time delta
      // const dt = cursor.m_p2[0] - cursor.m_p1[0]
      // const dtStr = this.format_time_delta(dt)
      // Format text

      var priceText = "".concat(d$.toFixed(2), " (").concat(p, "%)"); // const timeText = dtStr
      // Setup text style

      ctx.font = this.$p.font;
      ctx.textAlign = 'right'; // Align to right end

      ctx.fillStyle = colors.text; // Calculate text dimensions - only show price text, not time

      var lines = [priceText]; // Removed timeText

      var lineHeight = 18;
      var textPadding = 15; // Padding from box edge
      // Determine measurement direction

      var dir = Math.sign(y2 - y1); // Position text outside the box based on direction
      ...
      var baseY = y2;
      var textOffset = dir > 0 ? textPadding : -(textPadding + lineHeight * (lines.length - 1)); // Use right edge (x2) directly - textAlign 'right' ensures text ends at x2

      var textX = x2; // Draw text directly without background
    ...
    key: "format_time_delta",
    value: function format_time_delta(dt) {
      var sign = dt < 0 ? '-' : '';
      var abs = Math.abs(dt);
      var second = 1000;
      var minute = second * 60;
      ...
      if (abs < minute) {
        return "".concat(sign).concat(Math.floor(abs / second), "s");
      } else if (abs < hour) {
        return "".concat(sign).concat(Math.floor(abs / minute), "m");
      } else if (abs < day) {
        var h = Math.floor(abs / hour);
        var m = Math.floor(abs % hour / minute);
        return m > 0 ? "".concat(sign).concat(h, "h ").concat(m, "m") : "".concat(sign).concat(h, "h");
      } else {
        var d = Math.floor(abs / day);

        var _h = Math.floor(abs % day / hour);

        return _h > 0 ? "".concat(sign).concat(d, "d ").concat(_h, "h") : "".concat(sign).concat(d, "d");
      }
    }
  }, {
    key: "hide",
...
/* harmony default export */ const Chartvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    cursor_changed: function cursor_changed(e) {
      if (e.mode) this.cursor.mode = e.mode; // Explicitly update cursor position if provided (allow null for clearing, reject NaN)
  ...
  data: function data() {
    return {
      ...
      cursor: {
        ...
        handle_x: null,
        handle_y: null,
        // Mobile control handle position
        // Mobile measurement state
        measuring: false,
        m_p1: null,
        // First measurement point [t, y$]
